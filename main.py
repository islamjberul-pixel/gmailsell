...
ADMIN_ID = str(os.getenv("ADMIN_ID")) # String করে নিলাম
ADMIN_WHATSAPP = "8801796103936"

# ===== FIREBASE CONNECT - Error Handle সহ =====
try:
...
def start(message):
    ...
    bot.send_message(message.chat.id, f"Welcome {message.from_user.first_name} to *BK71 CLUB Bot*!", reply_markup=main_menu())
...
def handler(message):
    ...
    if text == "💰 Balance":
        ...
        stock_ref = db.collection('gmail_stock').limit(20).stream()
        stock = [doc.to_dict() for doc in stock_ref]
        if not stock:
            bot.send_message(chat_id, "❌ Stock Empty! Admin কে জানাও", reply_markup=main_menu()); return
...
এই Info দিয়ে Gmail খুলে Submit করো"""
    ...
    # ========== ADMIN PANEL ==========
    ...
    elif text == "📧 Pending Gmail" and user_id == ADMIN_ID:
        pending_ref = db.collection('pending').stream()
        count = 0
        for doc in pending_ref:
            ...
            bot.send_message(chat_id, msg, reply_markup=markup)
        if count == 0: bot.send_message(chat_id, "কোনো Pending নাই", reply_markup=admin_menu())

    elif text == "💸 Pending Withdraw" and user_id == ADMIN_ID:
        wd_ref = db.collection('withdraws').where('status', '==', 'Pending').stream()
        count = 0
        for doc in wd_ref:
            count +=1
            data = doc.to_dict()
            ...
            bot.send_message(chat_id, msg, reply_markup=markup)
        if count == 0: bot.send_message(chat_id, "কোনো Pending Withdraw নাই", reply_markup=admin_menu())

    elif text == "👥 All Users" and user_id == ADMIN_ID:
        users = list(db.collection('users').stream())
        bot.send_message(chat_id, f"Total Users: `{len(users)}`", reply_markup=admin_menu())

    elif text == "➕ Add Gmail Stock" and user_id == ADMIN_ID:
...
def callback(call):
    ...
    # ADMIN APPROVE/REJECT
    elif "approve_" in call.data and user_id == ADMIN_ID:
        ...
        u = u_ref.get().to_dict()
        # BUG FIX: Hold + Main Balance আলাদা
        new_hold = u['hold'] + data['price']
        new_accounts = u['accounts']
        new_accounts.append({'email':data['email'],'pass':data['pass'],'price':data['price'],'status':'Hold'})
        u_ref.update({'hold': new_hold, 'accounts': new_accounts})
    ...
    elif "reject_" in call.data and user_id == ADMIN_ID:
        db.collection('pending').document(call.data.split("_")[1]).delete()
        bot.send_message(call.message.chat.id, "❌ Rejected")

    elif "paid_" in call.data and user_id == ADMIN_ID:
        wd_doc = db.collection('withdraws').document(call.data.split("_")[1]).get()
        wd_data = wd_doc.to_dict()
        # BUG FIX: Paid করার সময় Hold থেকে Main এ আনা
        u_ref = db.collection('users').document(wd_data['user_id'])
        u = u_ref.get().to_dict()
        u_ref.update({
            'hold': u['hold'] - wd_data['amount'],
            'balance': u['balance'] - wd_data['amount']
        })
        ...
        bot.send_message(wd_data['user_id'], f"✅ তোমার `{wd_data['amount']}` BDT Withdraw Paid হয়েছে")
...
def get_amount(message):
    ...
    try:
        ...
        if amount < 25:
            bot.send_message(message.chat.id, "❌ Min 25 BDT");
            del user_temp[user_id]
            return
        charge = amount * 0.03
        net = amount - charge
        bot.send_message(message.chat.id, f"Amount: `{amount}` BDT\nCharge: `{charge:.2f}` BDT\nNet: `{net:.2f}` BDT\nNumber দাও:")
...
def get_number(message):
    ...
    # BUG FIX: Request এর সময় শুধু Balance থেকে কাটবে, Hold থেকে না
